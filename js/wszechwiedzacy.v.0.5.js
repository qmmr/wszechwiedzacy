...
function hoverO() { $(this).addClass("ui-state-hover"); }
function hoverT() { $(this).removeClass("ui-state-hover"); }
function mDown() { $(this).addClass("ui-state-active"); }
function mUp() { $(this).removeClass("ui-state-active"); }
// main object
var wszechwiedzacy = {
    // placeholder for site URL
    site_url : {},
   // shows current location for debugging
   curLoc : function() { alert("wszechwiedzacy.curLoc -> "+window.location); },
    // initialized on every page
    init : function() {
	
	// checks if we're developing on localhost or live online
	(window.location.hostname == "localhost") ? wszechwiedzacy.site_url = "http://localhost/wszechwiedzacy/" : wszechwiedzacy.site_url = "http://wszechwiedzacy.com/";
	...
	$("#login_dialog").dialog({
		...
		// my_form.find("#email").val("this is not email");
			...
			$.ajax({
				...
				if( email == "sent" ) {
					...
					validator.showErrors({"email": "To nie jest poprawny adres email!"});
				     ...
				     } else if( email == "not posted" ) {
					
					var validator = $("#forgot_form").validate();
					validator.showErrors({"email": "Musisz podać adres email!"});
					
				     } else if( email == "database empty" ) {
				       
					var validator = $("#forgot_form").validate();
					validator.showErrors({"email": "Nie ma takiego adresu w naszej bazie!"});
					
				     } else if (email == "mail failed") {
	...
	$("#register_dialog").dialog({
		...
		Zarejestruj: function() {
			...
			rules: {			     
			    reg_username: { required: true, rangelength: [3, 14] },
			    reg_password: { required: true, rangelength: [4, 14] },
			    reg_password2: { required: true, equalTo: '#reg_password' },
			    email: { required: true, email: true },
			    year: "required",
			    month:"required",
			    day:"required",
		       ...
		       $.ajax({
				...
				if( data.user == "valid" && data.password == "valid" && data.email == "sent" && data.mysql == "valid" && data.recaptcha == "valid") {
				    ...
				    var cd = setTimeout(function(){
				       ...
				       $(".info_block").animate({"height": "toggle", "opacity": "toggle"},2000,"easeInQuint",function() {
					    $(this).remove();
				       });
				    ...
				    if( data.user == "taken" ) {
				       ...
				       validator.showErrors({"reg_username": "Ten nick jest już zarejestrowany!"});
	...
	$("#logout").click(function(e) {
		...
		type: "POST",
		url: wszechwiedzacy.site_url+"includes/logowanie.php",
		success: function( data ) { window.location = "http://wszechwiedzacy.com"; },
	...
	init : function() {
	    ...
	    */
	    var mc = $("#mainContent").expose({
	       ...
	       loadSpeed: 0,
	       api: true,
	       onBeforeClose: function(e) { qd.dialog("open"); } // if we click on the grey area a quit dialog pops
		...
		title: 'Uwaga!',
		buttons: {
		    Tak: function() {window.location.replace(wszechwiedzacy.site_url);},
		    Nie: function() {qd.dialog("close"); mc.load();} // mc.load loads expose tool		     
		} // end of buttons
		...
		switch(e.keyCode) {
		    case 49: case 97: $("label:eq(0)").click(wszechwiedzacy.gra.labelCheck()); break;
		    case 50: case 98: $("label:eq(1)").click(wszechwiedzacy.gra.labelCheck()); break;
		    case 51: case 99: $("label:eq(2)").click(wszechwiedzacy.gra.labelCheck()); break;
		    case 52: case 100: $("label:eq(3)").click(wszechwiedzacy.gra.labelCheck()); break;
		    case 13:
	...
	labelCheck : function() {
	    ...
	    if($(this).hasClass("radio_empty") && $("form label.radio_check").length == 0) {
		
		// if all labels are unchecked
		$(this).toggleClass("radio_empty").toggleClass("radio_check");
		$("#submitButton").fadeIn(500);
		// select correct input
		$(this).next().attr("selected", "selected");
		
	    } else if($(this).hasClass("radio_empty") && $("form label.radio_check").length != 0) {
		
		// if click on unchecked but there are other checked fields
		$("form label.radio_check").toggleClass("radio_check").toggleClass("radio_empty").next().removeAttr("selected");
		$(this).toggleClass("radio_check").toggleClass("radio_empty");
		$(this).next().attr("selected", "selected");
		
	    } else {
		
		// if we click on button that has class radio_checked we uncheck it and hide submit button
		$("#submitButton").hide();
		$(this).toggleClass( "radio_check" ).toggleClass("radio_empty");
	...
	setButtons : function() {
		...
		$(this).click(function(e) {
			...
			case "wprowadzenie": alert("zróbmy jakieś wprowadzenie"); break;
			case "rejestracja":
			    e.preventDefault();
			    my_form = $("#register_dialog");
			    $("#register_dialog").dialog("open");
	    ...
	    }).hover(hoverO, hoverT).mouseup(mUp).mousedown(mDown); // end of each function
	    
	}, // end wszechwiedzacy.gra.setButtons
	setupRadioButtons : function() {
	     
	     // checks all input of type radio if they have class radioNormal and if
	     // true they are removed and radioHidden are added
	    ($("form input:radio").hasClass("radioNormal")) ? $("form input:radio").removeClass("radioNormal").addClass("radioHidden") : false;
	    
	    // next all labels receive click function that checks if they have class radio_check
	    $("form label").click(function() {
	     
		// if label has class radio_empty and no other label has class
		// radio_check it switches the classes fades in submit button
		// and assigns attribute selected to the next object (after label there
		// is input type radio)
		if($(this).hasClass("radio_empty") && $("form label.radio_check").length == 0) {
	...
	showNextQuestion : function() {
		...
		success: function(data) { wszechwiedzacy.gra.loadTheQuestion(data); },
	...
	loadTheQuestion : function(data) {
	    ...
	    $("#pytanieWrap").fadeIn(500, function(){pytanie.fadeIn(500, callback);});
	    $("#loaderContainer").hide();			// hides the animation gif
	...
	init : function() {
		   ...
		   Dodaj: function(e) {
		       ...
		       if( $("#frmQuestionAdd").valid() ) {
			    ...
			    $.ajax({
			       ...
			       error: function(XMLHttpRequest, textStatus, errorThrown) { alert ("ajax call to add_question have failed, reason: "+textStatus); }
	   ...
	   $("#frmQuestionEdit").dialog({
		   ...
		   Aktualizuj: function(e) {
		       ...
		       } else if ($("#poprawna_edit").val() == 2) {
			   poprawna = $("#odp_b_edit").val();
		       } else if ($("#poprawna_edit").val() == 3) {
		       ...
		       if( $("#frmQuestionEdit").valid() ) {
			    ...
			    $.ajax({
			       ...
			       error: function(XMLHttpRequest, textStatus, errorThrown) { alert ("ajax call to edytuj_pytanie have failed, reason: "+textStatus); }
	       ...
	       buttons: {
		    Tak: function() {
		       ...
		       $.ajax({
			    ...
			    dataType: "json",
			    success: function(data) { $("#question_delete").dialog("close"); },	
			    error: function(XMLHttpRequest, textStatus, errorThrown) { alert ("ajax call to delete_question have failed, reason: "+textStatus); }
       ...
       addRow : function(id,runda,kategoria,tresc) {
	    wszechwiedzacy.admin.oTable.fnAddData( [
	       "<td name=\"numer\">"+id+"</td>",
	       "<td name=\"runda\">"+runda+"</td>",
	       "<td class=\"tla\" name=\""+kategoria+"\">"+kategoria+"</td>",
	       "<td class=\"tla\" name=\""+tresc+"\">"+tresc+"</td>",
	       "<td class=\"edit\"><a id=\""+id+"\" class=\"edit\" href=\"\"><span class=\"edit\" title=\"Edytuj pytanie\"></span></a></td>",
	       "<td class=\"delete\"><a id=\""+id+"\" class=\"delete\" href=\"\"><span class=\"delete\" title=\"Skasuj pytanie\"></span></a></td>"
	    ] );
       }, // end of addRow
	    ...
	    $("a.edit").each(function() {
	       ...
	       $(this).click(function(e) {
		    ...
		    $.ajax({
		       ...
		       error: function(XMLHttpRequest, textStatus, errorThrown) { alert ("ajax call to get_question have failed, reason: "+textStatus); }
       ...
       pytania : function() { alert("empty pytania"); } // end of admin.pytania
    ...
    kontakt : {
       ...
       init : function() {
		...
		var validator = $("#frmKontakt").validate({
			...
			email: { required: true, email: true },
			temat: "required",
			mainMessage: { required: true, rangelength: [15, 255] }
		    },
		    messages: {
		...
		if( $("#frmKontakt").valid() ) {
			...
			success: function( data ) {
				...
				$(".formWrap").fadeOut(1000, function() {
				    ...
				    // alert("Hidden");
				    $("#mainContent").html("<div class=\"emailConfirmation\"><span class=\"checkmark\"></span><h1>Email został wysłany.</h1><p>Dziękuję za kontakt. Postaram się odpowiedzieć w przeciągu 24h!</p><div class=\"clearfloat\"></div></div>").hide().fadeIn(500, function(){ $(".checkmark").animate({top: +70}, 750, "easeOutBounce"); }); // end of animate
				...
				if(n != 0) {
				    ...
				    $("#recaptcha_msg").css({"height":"0px"}).animate(
				       {height: "24px"},
				       500,
				       "easeOutBounce"
				    );
			...
			error: function(XMLHttpRequest, textStatus, errorThrown){ alert ("błąd podczas przesyłania wiadomości: "+textStatus); } // end of error
